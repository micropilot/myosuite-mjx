...
class TrackEnv(PipelineEnv):
    ...
    def _load_reference_motion(
        ...
        self.object_bid = mujoco.mj_name2id(
            mj_model, mujoco.mjtObj.mjOBJ_BODY, object_name
        )
    ...
    def compute_reward(self, curr_ref, data):
        ...
        obj_reward = jp.exp(-self.obj_err_scale * obj_com_err) * jp.exp(
            -self.obj_err_scale * obj_rot_err
        )
        ...
        hand_qvel_err = (
            ...
            else (curr_hand_qvel - targ_hand_qvel)
        )
        qpos_reward = jp.exp(-self.qpos_err_scale * self.norm2(hand_qpos_err))
        qvel_reward = (
            jp.array([0])
            if hand_qvel_err is None
            else jp.exp(-self.qvel_err_scale * self.norm2(hand_qvel_err))
        )
        ...
        metrics = {
            "pose": jp.float32(pose_reward + vel_reward),
        ...
        reward = jp.sum(
            jp.array(
                [wt * metrics[key] for key, wt in self.reward_weights_dict.items()]
            ),
            axis=0,
        )
