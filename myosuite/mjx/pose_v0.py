...
class PoseEnvV0(MjxEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if viz_site_targets:
            for site in viz_site_targets:
                self.tip_sids.append(
                    mujoco.mj_name2id(mj_model, mujoco.mjtObj.mjOBJ_SITE, site)
                )
                self.target_sids.append(
                    mujoco.mj_name2id(
                        mj_model, mujoco.mjtObj.mjOBJ_SITE, site + "_target"
                    )
                )

        # TODO: Ignore normalize act for now

        self.q_pos_init = jp.array([0.0] * 23)
        ...
        self.state_dim = self._get_obs(
            data.data, jp.array([0, 0, 0]), jp.array([0, 0, 0])
        ).shape[-1]
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        observation = self._get_obs(
            data.data, state.info["target_left"], state.info["target_right"]
        )
