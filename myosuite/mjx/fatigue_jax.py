...
class CumulativeFatigueJAX:
    ...
    def __init__(self, mj_model, frame_skip=1, seed=None):
        ...
        )  # Adjusted time step based on frame skip
        ...
        self.rng_key = (
            jax.random.PRNGKey(seed) if seed is not None else jax.random.PRNGKey(0)
        )
    ...
    def reset(self, fatigue_reset_vec=None, fatigue_reset_random=False):
        ...
        if fatigue_reset_random:
            ...
            ), f"Invalid fatigue vector length (expected {self.na}, got {len(fatigue_reset_vec)})"
