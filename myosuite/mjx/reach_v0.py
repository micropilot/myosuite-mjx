...
class ReachEnvV0(BaseV0):
    ...
    def reset(self, rng: jax.Array = None):
        ...
        new_site_pos = self.sys.site_pos.copy()  # Assuming site_pos is a mutable type like a list or numpy array
        for site, span in self.target_reach_range.items():
            ...
            new_site_pos = new_site_pos.at[sid].set(jax.random.uniform(subkey, shape=span[0].shape, minval=span[0], maxval=span[1]))
    ...
    def compute_reward(self, pipeline_state: State) -> dict:
        ...
        reward = jp.sum(jp.array([metrics[k] * v for k, v in self.weighted_reward_keys.items()]))
