...
class MyoLegReflex(object):
    ...
    def get_obs_dict(self):
        ...
        pelvis_vel = np.hstack(
            (
                ...
                self.env.sim.data.get_body_xvelr("pelvis").copy(),
            )
        )  # Velocity might need to be negative in Pitch (y-axis)

        # GRF from foot contact sensor values
        temp_right = (
            self.env.sim.data.get_sensor("r_foot").copy()
            + self.env.sim.data.get_sensor("r_toes").copy()
        )
        temp_left = (
            self.env.sim.data.get_sensor("l_foot").copy()
            + self.env.sim.data.get_sensor("l_toes").copy()
        )
        ...
        sensor_data["r_leg"]["load_ipsi"] = temp_right / (
            np.sum(self.env.sim.model.body_mass) * 9.8
        )
        ...
        for s_leg, s_legc in zip(["r_leg", "l_leg"], ["l_leg", "r_leg"]):
            ...
            sensor_data[s_leg]["phi_hip"] = np.pi - self.env.sim.data.get_joint_qpos(
                f"hip_flexion_{s_leg[0]}"
            )
            sensor_data[s_leg]["phi_knee"] = np.pi - self.env.sim.data.get_joint_qpos(
                f"knee_angle_{s_leg[0]}"
            )
            sensor_data[s_leg]["phi_ankle"] = (
                0.5 * np.pi
                - self.env.sim.data.get_joint_qpos(f"ankle_angle_{s_leg[0]}")
            )
            sensor_data[s_leg]["dphi_knee"] = -1 * self.env.sim.data.get_joint_qvel(
                f"knee_angle_{s_leg[0]}"
            )
    ...
    def run_reflex_step(self):
        ...
        if (
            self.env.sim.data.get_body_xpos("pelvis")[2] < 0.65
    ...
    def _set_muscle_groups(self):
        # ----- Gluteus group -----
        glu_r = [
            self.env.sim.model.actuator_names.index("glmax1_r"),
        ...
        glu_l = [
            self.env.sim.model.actuator_names.index("glmax1_l"),
        ...
        ham_r = [
            self.env.sim.model.actuator_names.index("semimem_r"),
            self.env.sim.model.actuator_names.index("semiten_r"),
            self.env.sim.model.actuator_names.index("bflh_r"),
        ]

        ham_l = [
            self.env.sim.model.actuator_names.index("semimem_l"),
        ...
        # ----- Gastrocnemius -----
        gas_r = [
            self.env.sim.model.actuator_names.index("gaslat_r"),
            self.env.sim.model.actuator_names.index("gasmed_r"),
        ]

        gas_l = [
            self.env.sim.model.actuator_names.index("gaslat_l"),
        ...
        # ----- Soleus -----
        sol_r = [
            self.env.sim.model.actuator_names.index("soleus_r"),
        ...
        sol_l = [
            self.env.sim.model.actuator_names.index("soleus_l"),
        ...
        hfl_r = [
            self.env.sim.model.actuator_names.index("psoas_r"),
            self.env.sim.model.actuator_names.index("iliacus_r"),
        ]

        hfl_l = [
            self.env.sim.model.actuator_names.index("psoas_l"),
        ...
        hab_r = [
            self.env.sim.model.actuator_names.index("piri_r"),
        ...
        hab_l = [
            self.env.sim.model.actuator_names.index("piri_l"),
        ...
        had_r = [
            self.env.sim.model.actuator_names.index("addbrev_r"),
        ...
        had_l = [
            self.env.sim.model.actuator_names.index("addbrev_l"),
        ...
        # ----- Vastius group -----
        vas_r = [
            self.env.sim.model.actuator_names.index("vasint_r"),
            self.env.sim.model.actuator_names.index("vaslat_r"),
            self.env.sim.model.actuator_names.index("vasmed_r"),
        ]

        vas_l = [
            self.env.sim.model.actuator_names.index("vasint_l"),
        ...
        temp_L0 = (
            self.env.sim.model.actuator_lengthrange[:, 0]
            - self.env.sim.model.tendon_lengthspring
        ) / self.env.sim.model.actuator_biasprm[:, 0]
    ...
    def _set_initial_pose(self, init_dict):
        ...
        for i in init_dict["joint_angles"].keys():
            self.env.sim.data.qpos[
                self.env.sim.model.joint_names.index(i) + temp_offset
    ...
    def update_footstep(self):
        ...
        r_contact = (
            True
            if (self.env.sim.data.get_sensor("r_foot").copy())
            > 0.1 * (np.sum(self.env.sim.model.body_mass) * 9.8)
            else False
        )
        l_contact = (
            True
            if (self.env.sim.data.get_sensor("l_foot").copy())
            > 0.1 * (np.sum(self.env.sim.model.body_mass) * 9.8)
            else False
        )
