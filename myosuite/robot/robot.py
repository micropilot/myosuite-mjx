        ...
        if self.is_hardware:
            ...
            for name, device in self.robot_config.items():
                if name == "default_robot":
                    ...
                    sen["act"] = (
                        self.sim.data.act.copy() if self.sim.model.na > 0 else None
                    )
    ...
    def sync_sims(self, source_sim, destination_sim, model=True, data=True):
        ...
        if model:
            if destination_sim.model.nsite > 0:
                ...
                destination_sim.model.site_quat[:] = source_sim.model.site_quat[
                    :
                ].copy()
            if destination_sim.model.nbody > 0:
                destination_sim.model.body_pos[:] = source_sim.model.body_pos[:].copy()
                destination_sim.model.body_quat[:] = source_sim.model.body_quat[
                    :
                ].copy()
        ...
        for name, device in self.robot_config.items():
            if name == "default_robot":
                if self._act_mode == "pos":
                    if normalized:
                        processed_controls = (
                            np.mean(self.sim.model.actuator_ctrlrange, axis=-1)
                            + controls
                            * (
                                self.sim.model.actuator_ctrlrange[:, 1]
                                - self.sim.model.actuator_ctrlrange[:, 0]
                ...
                for actuator in device["actuator"]:
                    ...
                    if self._act_mode == "pos":
                        ...
                        if velocity_limits:
                            last_obs = getattr(self.sim.data, actuator["data_type"])[
                                actuator["data_id"]
                            ]
