...
def render_camera_offscreen(
    cameras: list, width: int = 640, height: int = 480, device_id: int = 0, sim=None
):
    """
...
def main(model_path, cam_names, width, height, device_id):
    ...
    sim = MjSim(model)
    imgs = render_camera_offscreen(
        cameras=cam_names, width=width, height=height, device_id=device_id, sim=sim
    )
