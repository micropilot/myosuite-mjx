...
def randomize_appliances(model_path, np_random):
    ...
    model_xml = re.sub(
        "microwave_body\d.xml", "microwave_body{}.xml".format(opt), model_xml
    )
...
def randomize_layout(sim, np_random):
    ...
    # Counter layout
    layout = {
        "sink": {"L": [-1.620, 0, 0], "R": [0, 0, 0]},
        "island": {"L": [-0.020, 0, 0], "R": [1.92, 0, 0]},
    }
    counter_loc = ["L", "R"]
    ...
    app_xyz = {
        ...
        # Left pannel
        "LLT": {
            "pos": app_xy["LL"] + app_z["T"],
            "euler": [0, 0, 1.57],
            "accept": True,
        },
        "LRT": {
            "pos": app_xy["LR"] + app_z["T"],
            "euler": [0, 0, 1.57],
            "accept": True,
        },
        "LLM": {
            "pos": app_xy["LL"] + app_z["M"],
            "euler": [0, 0, 1.57],
            "accept": True,
        },
        "LRM": {
            "pos": app_xy["LR"] + app_z["M"],
            "euler": [0, 0, 1.57],
            "accept": False,
        },
        "LLB": {
            "pos": app_xy["LL"] + app_z["B"],
            "euler": [0, 0, 1.57],
            "accept": True,
        },
        "LRB": {
            "pos": app_xy["LR"] + app_z["B"],
            "euler": [0, 0, 1.57],
            "accept": True,
        },
        # Right pannel
        "RLT": {
            "pos": app_xy["RL"] + app_z["T"],
            "euler": [0, 0, -1.57],
            "accept": True,
        },
        "RRT": {
            "pos": app_xy["RR"] + app_z["T"],
            "euler": [0, 0, -1.57],
            "accept": True,
        },
        "RLM": {
            "pos": app_xy["RL"] + app_z["M"],
            "euler": [0, 0, -1.57],
            "accept": False,
        },
        "RRM": {
            "pos": app_xy["RR"] + app_z["M"],
            "euler": [0, 0, -1.57],
            "accept": True,
        },
        "RLB": {
            "pos": app_xy["RL"] + app_z["B"],
            "euler": [0, 0, -1.57],
            "accept": True,
        },
        "RRB": {
            "pos": app_xy["RR"] + app_z["B"],
            "euler": [0, 0, -1.57],
            "accept": True,
        },
    }
    ...
    for body_name in ["slidecabinet", "hingecabinet", "microwave"]:
        ...
        sim.model.body_pos[bid] = np.array(app_xyz[sel_grid]["pos"]) + np.array(
            body_offset[body_name]["pos"]
        )
        sim.model.body_quat[bid] = euler2quat(
            np.array(app_xyz[sel_grid]["euler"])
            + np.array(body_offset[body_name]["euler"])
        )
        # mark occupied
...
def randomize_visuals(sim, np_random):
    # randomize colors
    sim.model.geom_rgba[:, :] = np_random.uniform(
        size=sim.model.geom_rgba.shape, low=0.4, high=0.8
    )
...
@click.option(
    ...
    "--model_path",
    type=str,
    help="full path of environment xml to load",
    required=True,
)
