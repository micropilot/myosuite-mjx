...
class MujocoEnv(gym.Env, gym.utils.EzPickle, ObsVecDict):
    ...
    def __init__(
        self, model_path, obsd_model_path=None, seed=None, env_credits=DEFAULT_CREDIT
    ):
        """
        Create a gym env
        INPUTS:
        ...
        self.sim_obsd = (
            SimScene.get_sim(obsd_model_path) if obsd_model_path else self.sim
        )
        self.sim.forward()
    ...
    def _setup(
        ...
        **kwargs,  # Additional arguments
        ...
        if self.normalize_act:
            ...
            actuated_jnt_ids = self.sim.model.actuator_trnid[
                self.sim.model.actuator_trntype == self.sim.lib.mjtTrn.mjTRN_JOINT, 0
            ]  # dm
            ...
            self.init_qpos[linear_actuated_jnt_qposids] = np.mean(
                self.sim.model.jnt_range[linear_actuated_jnt_ids], axis=1
            )
    ...
    def get_env_state(self):
        ...
        site_quat = (
            self.sim.model.site_quat[:].copy() if self.sim.model.nsite > 0 else None
        )
