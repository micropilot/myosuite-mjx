...
class CumulativeFatigue:
    ...
    def __init__(self, mj_model, frame_skip=1, seed=None):
        self._r = (
            10 * 15
        )  # Recovery time multiplier i.e. how many times more than during rest intervals https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6092960/ (factor 10 to compensate for 0.1 below)
        ...
        self._R = (
            0.1 * 0.00094
        )  # Recivery coefficients (default parameter was identified for elbow torque https://pubmed.ncbi.nlm.nih.gov/22579269/; factor 0.1 to get an approx. 1% R/F ratio)
        # self.na = mj_model.na
        self._dt = (
            mj_model.opt.timestep * frame_skip
        )  # dt might be different from model dt because it might include a frame skip
        muscle_act_ind = mj_model.actuator_dyntype == mujoco.mjtDyn.mjDYN_MUSCLE
        self.na = sum(
            muscle_act_ind
        )  # WARNING: here, self.na denotes the number of muscle actuators only!
        self._tauact = np.array(
            [
    ...
    def reset(self, fatigue_reset_vec=None, fatigue_reset_random=False):
        if fatigue_reset_random:
            ...
            ), "Cannot use 'fatigue_reset_vec' if fatigue_reset_random=False."
            ...
            if fatigue_reset_vec is not None:
                ...
                ), f"Invalid length of initial/reset fatigue vector (expected {self.na}, but obtained {len(fatigue_reset_vec)})."
