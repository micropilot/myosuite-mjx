...
class RunTrack(WalkEnvV0):
    ...
    def _setup(
        ...
        **kwargs,
        ...
        self.OSL_CTRL = MyoOSLController(
            np.sum(self.sim.model.body_mass),
            init_state="e_stance",
    ...
    def get_reward_dict(self, obs_dict):
        ...
        act_mag = (
            np.mean(np.square(self.obs_dict["act"])) if self.sim.model.na != 0 else 0
        )
        pain = self.get_pain()
    ...
    def _get_actuator_params(self):
        ...
        for actu in actuators:
            ...
            self.ACTUATOR_PARAM[actu]["Fmax"] = (
                np.max(self.sim.model.actuator(actu).ctrlrange)
                * self.sim.model.actuator(actu).gear[0]
            )
    ...
    def initializeFromData(self):
        ...
        for joint in self.gait_cycle_headers.keys():
            if joint not in [
                ...
                self.init_qpos[self.sim.model.joint(joint).qposadr[0]] = self.INIT_DATA[
    ...
    def _append_osl_actions(self, mus_actions, is_normalized):
        ...
        for jnt in ["knee", "ankle"]:
            ...
            osl_ctrl = (
                osl_torque[jnt]
                / self.sim.model.actuator(f"osl_{jnt}_torque_actuator").gear[0]
            )

            # clip for control limits
            min_ctrl = self.sim.model.actuator(f"osl_{jnt}_torque_actuator").ctrlrange[
                0
            ]
            max_ctrl = self.sim.model.actuator(f"osl_{jnt}_torque_actuator").ctrlrange[
                1
            ]
    ...
    def get_osl_sens(self):
        ...
        osl_sens_data["knee_angle"] = (
            self.sim.data.joint("osl_knee_angle_r").qpos[0].copy()
        )
        osl_sens_data["knee_vel"] = (
            self.sim.data.joint("osl_knee_angle_r").qvel[0].copy()
        )
        osl_sens_data["ankle_angle"] = (
            self.sim.data.joint("osl_ankle_angle_r").qpos[0].copy()
        )
        osl_sens_data["ankle_vel"] = (
            self.sim.data.joint("osl_ankle_angle_r").qvel[0].copy()
        )
        osl_sens_data["load"] = (
            -1 * self.sim.data.sensor("r_osl_load").data[1].copy()
        )  # Only vertical
