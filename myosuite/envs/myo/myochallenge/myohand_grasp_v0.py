...
class BimanualGraspEnvV1(BaseV0):
    DEFAULT_OBS_KEYS = ["time", "myohand_qpos", "myohand_qvel", "object_qpos", "object_qvel", "touching_body"]
    ...
    def get_reward_dict(self, obs_dict):
        ...
        reach_dist = np.abs(np.linalg.norm(obs_dict['reach_err'], axis=-1))[0][0]
        # print ("reach_dist: ", reach_dist)
        ...
        act = (np.linalg.norm(obs_dict['act'], axis=-1) / self.sim.model.na)[0][0] if self.sim.model.na != 0 else 0
        # print ("act: ", act)
        fin_keys = ['fin0', 'fin1', 'fin2', 'fin3', 'fin4']
        fin_open = sum(np.linalg.norm(obs_dict[fin] - obs_dict['palm_pos'], axis=-1) for fin in fin_keys)[0][0]
        fin_dis = sum(np.linalg.norm(obs_dict[fin] - obs_dict['obj_pos'], axis=-1) for fin in fin_keys)[0][0]

        elbow_err = (5 * np.exp(-10 * (obs_dict['elbow_fle'][0] - 1.) ** 2) - 5)[0][0]
        # print ("fin_open: ", fin_open, "fin_dis: ", fin_dis, "lift_bonus: ", elbow_err, "lift_height: ", lift_height)
        
        touching_vec = obs_dict["touching_body"][0][0] if obs_dict['touching_body'].ndim == 3 else obs_dict['touching_body']
        # print ("solved", self.goal_touch >= self.TARGET_GOAL_TOUCH)
        # print ("done", self._get_done(obj_pos[-1]))
        ...
        rwd_dict = collections.OrderedDict(
            (
                ...
                ("sparse", 0),
                ("goal_dist", 0), 
                ("solved", self.goal_touch >= self.TARGET_GOAL_TOUCH),
...
class ContactTrajIssue(enum.Enum):
    MYO_SHORT = 0
    NO_GOAL = 1  # Maybe can enforce implicitly, and only declare success is sufficient consecutive frames with only
    # goal contact.
    ENV_CONTACT = 2
