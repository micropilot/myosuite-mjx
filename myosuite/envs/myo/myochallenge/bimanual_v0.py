...
class BimanualEnvV1(BaseV0):
    ...
    def _setup(
        ...
        **kwargs,
        ...
        self.obj_mass_range = (
            {
                "low": self.sim.model.body_mass[self.obj_bid] + obj_mass_change[0],
        ...
        self.obj_friction_range = (
            {
                "low": self.sim.model.geom_friction[self.obj_gid] - obj_friction_change,
                "high": self.sim.model.geom_friction[self.obj_gid]
                + obj_friction_change,
            }
    ...
    def __center_box_mesh(self):
        ...
        self.sim.model.mesh_vert[self.obj_vert_addr] = r.apply(
            self.sim.model.mesh_vert[self.obj_vert_addr]
        )
        self.sim.model.mesh_normal[self.obj_vert_addr] = r.apply(
            self.sim.model.mesh_normal[self.obj_vert_addr]
        )
        self.sim.model.geom(self.obj_gid - 1).quat = [1, 0, 0, 0]
        self.sim.model.mesh_vert[self.obj_vert_addr] += (
            self.sim.model.geom(self.obj_gid - 1).pos
            - self.sim.model.geom(self.obj_gid).pos
        )[None, :]

        self.sim.model.geom(self.obj_gid - 1).pos = self.sim.model.geom(
            self.obj_gid
        ).pos
    ...
    def get_obs_dict(self, sim):
        ...
        obs_dict["pros_hand_qpos"] = sim.data.qpos[
        ...
        obs_dict["Rpalm_pos"] = (
            sim.data.site_xpos[self.Rpalm1_sid] + sim.data.site_xpos[self.Rpalm2_sid]
        ) / 2

        obs_dict["MPL_ori"] = mat2euler(
            np.reshape(self.sim.data.site_xmat[self.Rpalm1_sid], (3, 3))
        )
    ...
    def reset(self, **kwargs):
        ...
        if self.obj_friction_range:
            self.sim.model.geom_friction[self.obj_gid] = self.np_random.uniform(
                **self.obj_friction_range
            )
