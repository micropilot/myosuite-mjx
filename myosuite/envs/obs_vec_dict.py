...
class ObsVecDict:
    ...
    def __init__(self, obsvec_cachesize=5):
        self.key_idx = {}
    ...
    def add_obsvec_to_cache(self, t, obsvec, check_timeStamps=True):
        ...
        if (
            check_timeStamps
            and len(self._obsvec_cache) > 0
            and t == self._obsvec_cache[-1][0]
        ):
    ...
    def initialize(self, obs_dict, ordered_obs_keys):
        ...
        self.obsvec_cache_flush(
            t, obsvec
        )  # populate the cache with initial obsvec  values
