...
def _update_env_spec_kwarg(env_variant_specs, variants, override_keys):
    env_variant_specs._kwargs, variants_update_keyval_str = update_dict(
        env_variant_specs._kwargs, variants, override_keys=override_keys
    )
...
def update_dict(base_dict: dict, update_dict: dict, override_keys: list = None):
    ...
    if override_keys:
        base_dict = {
            key: item for key, item in base_dict.items() if key not in override_keys
        }
...
def register_env_variant(
    env_id: str, variants: dict, variant_id=None, silent=False, override_keys=None
):
    """
    ...
    assert (
        env_id in gym_registry_specs().keys()
    ), "ERROR: {} not found in env registry".format(env_id)
    ...
    if "max_episode_steps" in variants.keys():
        ...
        env_variant_id = env_variant_id + "-hor_{}".format(
            env_variant_specs.max_episode_steps
        )
        del variants["max_episode_steps"]

    # merge specs._kwargs with variants
    variants_update_keyval_str = _update_env_spec_kwarg(
        env_variant_specs, variants, override_keys
    )
    env_variant_id += variants_update_keyval_str

    # finalize name and register env
    env_variant_specs.id = (
        env_variant_id + env_variant_specs.id[-3:] if variant_id is None else variant_id
    )
    register(
...
if __name__ == "__main__":
    ...
    variant_env_name = register_env_variant(
        env_id=base_env_name, variants=base_env_variants
    )
    variant_overide_env_name = register_env_variant(
        env_id=base_env_name, variants=base_env_variants, override_keys="obs_keys_wt"
    )  # Instead of updating via merge, obs_keys_wt key will be completeley overwritten
