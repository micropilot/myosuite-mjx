...
class MyoLegReflex(object):
    ...
    def get_obs_dict(self):
        ...
        temp_seg_vel = self.env.sim.data.object_velocity(
        ...
        # GRF from foot contact sensor values
        temp_right = (
            self.env.sim.data.sensor("r_foot").data[0].copy()
            + self.env.sim.data.sensor("r_toes").data[0].copy()
        )
        temp_left = (
            self.env.sim.data.sensor("l_foot").data[0].copy()
            + self.env.sim.data.sensor("l_toes").data[0].copy()
        )
        ...
        for s_leg, s_legc in zip(["r_leg", "l_leg"], ["l_leg", "r_leg"]):
            ...
            sensor_data[s_leg]["phi_hip"] = (
                np.pi - self.env.sim.data.jnt(f"hip_flexion_{s_leg[0]}").qpos[0].copy()
            )
            sensor_data[s_leg]["phi_knee"] = (
                np.pi - self.env.sim.data.jnt(f"knee_angle_{s_leg[0]}").qpos[0].copy()
            )
            sensor_data[s_leg]["phi_ankle"] = (
                0.5 * np.pi
                - self.env.sim.data.jnt(f"ankle_angle_{s_leg[0]}").qpos[0].copy()
            )
            sensor_data[s_leg]["dphi_knee"] = (
                self.env.sim.data.jnt(f"knee_angle_{s_leg[0]}").qvel[0].copy()
            )
            ...
            sensor_data[s_leg]["alpha_f"] = (
                -1 * self.env.sim.data.jnt(f"hip_adduction_{s_leg[0]}").qpos[0].copy()
            ) + 0.5 * np.pi
    ...
    def run_reflex_step(self):
        ...
        if (
            self.env.sim.data.body("pelvis").xpos[2] < 0.65
    ...
    def _set_muscle_groups(self):
        # ----- Gluteus group -----
        glu_r = [
            self.env.sim.model.actuator("glmax1_r").id,
        ...
        glu_l = [
            self.env.sim.model.actuator("glmax1_l").id,
        ...
        ham_r = [
            self.env.sim.model.actuator("semimem_r").id,
            self.env.sim.model.actuator("semiten_r").id,
            self.env.sim.model.actuator("bflh_r").id,
        ]

        ham_l = [
            self.env.sim.model.actuator("semimem_l").id,
        ...
        # ----- Gastrocnemius -----
        gas_r = [
            self.env.sim.model.actuator("gaslat_r").id,
            self.env.sim.model.actuator("gasmed_r").id,
        ]

        gas_l = [
            self.env.sim.model.actuator("gaslat_l").id,
        ...
        # ----- Soleus -----
        sol_r = [
            self.env.sim.model.actuator("soleus_r").id,
        ...
        sol_l = [
            self.env.sim.model.actuator("soleus_l").id,
        ...
        hfl_r = [
            self.env.sim.model.actuator("psoas_r").id,
            self.env.sim.model.actuator("iliacus_r").id,
        ]

        hfl_l = [
            self.env.sim.model.actuator("psoas_l").id,
        ...
        hab_r = [
            self.env.sim.model.actuator("piri_r").id,
        ...
        hab_l = [
            self.env.sim.model.actuator("piri_l").id,
        ...
        had_r = [
            self.env.sim.model.actuator("addbrev_r").id,
        ...
        had_l = [
            self.env.sim.model.actuator("addbrev_l").id,
        ...
        # ----- Vastius group -----
        vas_r = [
            self.env.sim.model.actuator("vasint_r").id,
            self.env.sim.model.actuator("vaslat_r").id,
            self.env.sim.model.actuator("vasmed_r").id,
        ]

        vas_l = [
            self.env.sim.model.actuator("vasint_l").id,
        ...
        temp_L0 = (
            ...
            - self.env.sim.model.tendon_lengthspring[:, 0]
    ...
    def _set_initial_pose(self, init_dict):
        ...
        for joint_name in init_dict["joint_angles"].keys():
            self.env.sim.data.joint(joint_name).qpos[0] = init_dict["joint_angles"][
                joint_name
            ]
    ...
    def update_footstep(self):
        ...
        r_contact = (
            True
            if (self.env.sim.data.sensor("r_foot").data[0].copy())
            > 0.1 * (np.sum(self.env.sim.model.body_mass) * 9.8)
            else False
        )
        l_contact = (
            True
            if (self.env.sim.data.sensor("l_foot").data[0].copy())
            > 0.1 * (np.sum(self.env.sim.model.body_mass) * 9.8)
            else False
        )
